"""Paddle implementation of Logistic distribution functions."""
import paddle
...
def _logsumexp(x, axis):
    if len(x.shape) <= 4:
        ...
        new_shape = [np.prod(x_shape[:axis])] + [x_shape[axis]] + [np.prod(x_shape[axis + 1:])] # reshape to 3 dimensions
        _sum = paddle.logsumexp(x.reshape(new_shape), axis=1) 
        return _sum.reshape(x_shape[:axis] + x_shape[axis + 1:])

def _log_pdf(x, mean, log_scale):
    """Element-wise log density of the logistic distribution."""
    z = (x - mean) * paddle.exp(-log_scale)
...
def mixture_log_pdf(x, prior_logits, means, log_scales):
    ...
    log_ps = F.log_softmax(prior_logits, axis=1) \
        + _log_pdf(x.unsqueeze(1), means, log_scales)
    log_p = _logsumexp(log_ps, axis=1)
...
def mixture_inv_cdf(y, prior_logits, means, log_scales,
    ...
    def body(x_, lb_, ub_):
        cur_y = paddle.exp(mixture_log_cdf(x_, prior_logits, means,
                                          log_scales))
        #gt = (cur_y > y).type(y.dtype)
        gt = paddle.cast(cur_y > y, y.dtype)
        lt = 1 - gt
    ...
    x = paddle.zeros_like(y)
    max_scales = paddle.sum(paddle.exp(log_scales), axis=1, keepdim=True)
    lb = (means - 20 * max_scales).min(axis=1)
    ub = (means + 20 * max_scales).max(axis=1)
    diff = float('inf')
